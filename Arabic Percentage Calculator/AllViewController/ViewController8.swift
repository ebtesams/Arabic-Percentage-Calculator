    ...
    @IBAction func output(_ sender: Any) {
        if number1Txt.text?.isEmpty == false  && number2Txt.text?.isEmpty == false {
            if  !number1Txt.text!.isNumber || !number2Txt.text!.isNumber {
                ...
                resultLbl.text = String(format: "%.2f" , Functions.Equation(equationNumber: 8 , num1: number1Txt.text! ,num2: number2Txt.text!))
            }
        }
        else
        {
...
extension ViewController8: UITextFieldDelegate {
